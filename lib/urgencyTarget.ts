...
//
// Kept centralized so the order detail page, the /orders inline editor, and the new-order form
// all resolve identically regardless of which one triggers the save.
function monthName(year: number, month: number) {
  return new Date(Date.UTC(year, month - 1, 1)).toLocaleString('en-US', { month: 'long', timeZone: 'UTC' });
}

export function resolveUrgencyTargetDay(day: number, now: Date = new Date()): { date: string } | { error: string } {
...
export function normalizeUrgencyFields(
